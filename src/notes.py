...
def get_notes(db: Session = Depends(get_db), limit: int = 10, page: int = 1, search: str = ''):
    ...
    notes = db.query(model.Note).filter(
        model.Note.title.contains(search)).limit(limit).offset(skip).all()
...
def create_note(payload: schemas.NoteBaseSchema, db: Session = Depends(get_db)):
    new_note = model.Note(**payload.dict())
    db.add(new_note)
    db.commit()
    db.refresh(new_note)
...
def update_note(noteId: str, payload: schemas.NoteBaseSchema, db: Session = Depends(get_db)):
    note_query = db.query(model.Note).filter(model.Note.id == noteId)
    ...
    note_query.filter(model.Note.id == noteId).update(update_data,
...
def get_post(noteId: str, db: Session = Depends(get_db)):
    note = db.query(model.Note).filter(model.Note.id == noteId).first()
    if not note:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
